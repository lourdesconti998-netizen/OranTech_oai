...
// throughput UL, SNR, MCS UL y BLER UL.
      int bad_thp  = (g_has_ul_thp  && g_ul_thp_kbps < 10000.0);
      int bad_bler = (g_has_ul_bler && g_ul_bler_pct > 2.0);
      int bad_snr  = (g_has_ul_snr  && g_ul_snr_db < 35.0);
      int bad_mcs  = (g_has_ul_mcs  && g_ul_mcs < 20);

      if (bad_thp || bad_bler || bad_snr || bad_mcs) {
        estado = "MALO";
...
// Impresión del estado final del canal junto con las métricas
      ...
      if (strcmp(estado, "MALO") != 0) {
        printf("Plan de contingencia: No requerido.\n");
      } else {
        printf("Plan de contingencia: ");
        if (bad_thp)  printf("[THP bajo: priorizar PRBs UL, revisar scheduler y colas QoS] ");
        if (bad_snr)  printf("[SNR bajo: revisar potencia UL, interferencia, beamforming/handover] ");
        if (bad_mcs)  printf("[MCS bajo: usar MCS más robusto temporalmente y re-evaluar CQI] ");
        if (bad_bler) printf("[BLER alto: reforzar HARQ, bajar MCS objetivo y validar transporte] ");
        printf("\n");
      }
    }  
    counter++;
